...
- The complement of a permutation graph is a comparability graph.
...
- *Kőnig's infinity lemma* Let $G$ be an infinite graph, with its vertices partitioned into finite sets $V_i$ such that every $v ∈ V_i$ has a neighbour $f(v) ∈ V_{i-1}$.
...
- If $X$ is a hereditary class and the prime graphs from $X$ are strongly wqo by induced subgraphs, then $X$ is wqo by induced subgraphs. (Strongly wqo: *CHECK ME*)
