...
- If $A' ⊂ A$ and $B' ⊂ B$ are finite dimensional $K$-algebras, then $\Z_{A⊗B}(A'⊗B') = \Z_A(A')⊗\Z_B(B')$.
- If $A,B$ are finite dimensional central $K$-algebras, $A⊗B$ is a central $K$-algebra.
- If $A,B$ are finite dimensional simple $K$-algebras and $A$ is also central, then $A⊗B$ is simple.
- If $A$ is an $n$-dimensional central simple $K$-algebra, then there is a $K$-algebra isomorphism $A ⊗ A^\op ≅ M_n(K)$.
- If $A, B$ are finite dimensional division $K$-algebras and are Brauer-equivalent, then $A ≅ B$.
- Corollary: every Brauer-equivalence class has a unique rerpsentative which is a finite dimensional central simple division algebra.
- If $A$ is a finite dimensional simple $K$-algebra, $M,N$ are finitely generated $A$-modules and $\dim_K(M) = \dim_K(N)$ then $M ≅ N$ as $A$-modules.
- *Skolem-Noether* Let $K$ be a field, $A$ a finite dimensional central simple $K$-algebra, and $B_0, B_1$ simple subalgebras. Then any $K$-algebra isomorphism $B_0 → B_1$ extends to an inner $K$-algebra automorphism $(x \mapsto a^{-1}xa)$ of $A$, for some $a ∈ A$.
...
- $B ⊂ A$ finite dimensional simple $K$-algebras, and $A$ central. Then
  - $\Z_A(B)$ is simple.
  - $\dim_K(A) = \dim_K(B)\dim_K(\Z_A(B))$.
  - $\Z_A(\Z_A(B)) = B$, and $\Z(\Z_A(B)) = \Z(B)$.
- If $B ⊂ A$ are finite dimensional central simple $K$-algebras, there is an isomorphism of $K$-algebras $B⊗\Z_A(B)→A: b⊗t \mapsto bt$.
- $A$ a finite dimensional central division algebra over $K$ and $L ⊂ A$ a maximal subfield. Then for some $n$,
  - $\Z_A(L) = L$.
...
- *Frobenius* The only finite dimensional division algebras over $ℝ$ are $ℝ$, $ℂ$ and $ℍ$.
